...
**Calibrated against real packages, not just test cases.** Rules are measured against two corpora cloned from the AUR: ~158 installed packages, and the ~120 packages that ship a *hidden* install scriptlet — a population the first corpus barely contains, and therefore the one where the scriptlet rules had never actually been tested. That second run found 32 packages wrongly reported as malicious and led to five rule corrections: `PRIV-001` cannot mean anything inside a scriptlet that already runs as root; shipping a `.service` file and enabling it are how packaging works, not persistence; a config file naming `/etc/shadow` is data, not a script; and `chmod +x` on a binary the package itself installed is a permission fix, not an attack — the *download* is what matters, and a separate rule catches that. Both corpora now report zero false positives, and the worm is still flagged on non-overridable rules alone.
